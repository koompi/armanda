  ...
  return (
    <div className="card-highlight">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
        <div className="lg:col-span-2 bg-white dark:bg-gray-700 p-5 rounded-xl shadow-soft border border-gray-100 dark:border-gray-600">
          <div className="flex items-center mb-3">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-armandra" viewBox="0 0 20 20" fill="currentColor">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg text-center">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{result.total_requests.toLocaleString()}</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg text-center">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Successful</p>
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">{result.successful_requests.toLocaleString()}</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg text-center">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Failed</p>
              <p className="text-2xl font-bold text-red-600 dark:text-red-400">{result.failed_requests.toLocaleString()}</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg text-center">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Success Rate</p>
              <p className="text-2xl font-bold" style={{ color: successRate > 90 ? '#059669' : successRate > 70 ? '#d97706' : '#dc2626' }}>
        ...
        <div className="bg-white dark:bg-gray-700 p-5 rounded-xl shadow-soft border border-gray-100 dark:border-gray-600">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600 dark:text-gray-300">Min Response Time</span>
              <span className="font-medium">{result.min_response_time.toFixed(2)} ms</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600 dark:text-gray-300">Avg Response Time</span>
              <span className="font-medium">{result.avg_response_time.toFixed(2)} ms</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600 dark:text-gray-300">Max Response Time</span>
              <span className="font-medium">{result.max_response_time.toFixed(2)} ms</span>
            </div>
            <div className="border-t border-gray-200 dark:border-gray-600 pt-3 mt-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-300">Throughput</span>
                <span className="font-bold text-armandra-dark">{result.throughput.toFixed(2)} req/s</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-8 text-center text-sm text-gray-500 dark:text-gray-400">
        <p>Test completed at {new Date(result.timestamp * 1000).toLocaleString()}</p>
